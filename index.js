...
function getCurrentAge(person) {
  ...
  let m = today.month - person.dob_month;
   if(m < 0 || (m === 0 && today.day < person.dob_day)) {
      age--;
    }
    //console.log(age);
      return age;
...
function getOldestPerson(persons) {
  ...
  for (let i = 1; i < persons.length; i++) {
    if (persons[i].dob_year < oldestPerson.dob_year ||
    ( persons[i].dob_year === oldestPerson.dob_year &&
      persons[i].dob_month < oldestPerson.dob_month
    ) || (
      persons[i].dob_month === oldestPerson.dob_month &&
